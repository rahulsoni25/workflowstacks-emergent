  ...
  return (
    <div className="border-b border-[#1A1D1E]">
      <div className="container mx-auto px-4 py-3.5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-1.5 text-[13px] text-text-muted min-w-0 overflow-hidden">
          {trail.map((item, i) => (
            <span key={i} className="flex items-center gap-1.5 min-w-0">
              {i > 0 && <span className="text-slate-700">/</span>}
              {item.href ? (
                <Link href={item.href} className="text-text-muted hover:text-[#C6F24E] transition-colors whitespace-nowrap">
                  {item.label}
                </Link>
              ) : (
        ...
        {pill && !right && (
          <span className="text-[10.5px] font-mono tracking-wide text-text-muted border border-[#262B2D] rounded px-2 py-1 whitespace-nowrap shrink-0">
            {pill}
          </span>
        )}
