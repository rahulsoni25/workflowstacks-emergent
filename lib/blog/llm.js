...
export async function callLLM({ system, user, tier = 'cheap', model, maxTokens = 1200, temperature = 0.4, json = false, tag = 'blog' }) {
  ...
  if (prov.name === 'gemini') {
    ...
    // 429 from OpenRouter on a :free model usually means the shared upstream
    // pool is throttled ("temporarily rate-limited upstream"), not our quota.
    // Wait once; if it persists, run this call on Groq rather than fail the
    // whole daily run on it. Transient, so it does NOT flip openrouterDown.
    if (res.status === 429 && prov.name === 'openrouter') {
      await new Promise((r) => setTimeout(r, 20_000))
      res = await fetch(url, { method: 'POST', headers, body: JSON.stringify(body) })
    }
    if (!res.ok && prov.name === 'openrouter' && [401, 402, 403, 429].includes(res.status) && process.env.GROQ_API_KEY) {
      if (res.status !== 429) openrouterDown = true
      prov.name = 'groq'
...
export function parseJson(text) {
  ...
  // Reasoning models wrap their thinking in tags; strip it before looking for
  // the object. Fences and stray prose around the object are also common.
  const cleaned = text
  ...
  // Last resort for the two malformations free models actually produce:
  // trailing commas before a closing bracket, and a reply cut off by the
  // token budget (unbalanced braces). Repair the first; refuse the second so
  // the caller retries rather than acting on half a verdict.
  const repaired = slice.replace(/,\s*([}\]])/g, '$1')
  try { return JSON.parse(repaired) } catch (e) {
    const opens = (slice.match(/[{[]/g) || []).length, closes = (slice.match(/[}\]]/g) || []).length
    throw new Error(opens !== closes ? `LLM JSON truncated (${opens} opens, ${closes} closes)` : `LLM JSON invalid: ${e.message.slice(0, 80)}`)
  }
}
