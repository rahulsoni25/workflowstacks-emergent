...
// Note on what is deliberately NOT in the list above: `automation`,
// `workflow-automation`, `no-code`, `cli`, `developer-tools`. n8n-style
// workflow tooling genuinely belongs in this catalog, so the vocabulary has to
// reach it — but as a *certifying* topic, `automation` also admits a Docker
// image updater (watchtower: automation, docker, devops, update-checker) and a
// YouTube posting bot, and both reached the homepage rail that way. They are
// left to the free-text pass below, where they must co-occur with the repo's
// own wording rather than standing alone as a tag.
...
// `strict` drops the free-text fallback and demands a real topic tag. The
// free-text vocabulary has to include "automation"/"workflow" to catch genuine
// n8n and agent tooling, which also lets in things like a Docker image updater
// that describes itself as "automating container updates". That is an
// acceptable trade for a sitemap of hundreds; it is not acceptable for the six
// cards on the homepage, which are the strongest internal links the site has.
export function isAiRelevant(skill, { strict = false } = {}) {
  if (!skill) return false

  const topics = (Array.isArray(skill.github_topics) ? skill.github_topics : [])
    .map((t) => String(t).toLowerCase())
  for (const t of topics) {
    if (AI_TOPICS.has(t)) return true
  }
