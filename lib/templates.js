...
import icpFitChecker from '../templates/icp-fit-checker.json'
import objectionPrepCard from '../templates/objection-prep-card.json'
import meetingNoshowReducer from '../templates/meeting-noshow-reducer.json'
import aiSupportAgent from '../templates/ai-support-agent.json'
...
export const TEMPLATES = {
  ...
  'icp-fit-checker': {
    slug: 'icp-fit-checker',
    title: 'ICP Fit Checker',
    outcome: 'Score every lead against your ideal customer profile (1-5) with a reason — so you spend outreach time only on prospects who’ll actually buy.',
    persona: 'sales',
    deliverable_type: 'n8n',
    filename: 'workflowstacks-icp-fit-checker.n8n.json',
    setup_minutes: 5,
    requires: ['A free n8n account (cloud or self-hosted)', 'A Google account (for Sheets)', 'An OpenAI API key (any OpenAI-compatible key works)'],
    match_keywords: ['icp', 'lead', 'leads', 'qualify', 'score', 'scoring', 'fit', 'prospect', 'sales', 'customer', 'profile', 'target', 'list'],
    workflow: icpFitChecker,
  },
  'objection-prep-card': {
    slug: 'objection-prep-card',
    title: 'Objection Prep Card',
    outcome: 'For each upcoming deal, generate the 3 likely objections and strong, honest responses — so you walk into every call already prepared.',
    persona: 'sales',
    deliverable_type: 'n8n',
    filename: 'workflowstacks-objection-prep-card.n8n.json',
    setup_minutes: 5,
    requires: ['A free n8n account (cloud or self-hosted)', 'A Google account (for Sheets)', 'An OpenAI API key (any OpenAI-compatible key works)'],
    match_keywords: ['objection', 'objections', 'sales', 'call', 'deal', 'prep', 'prepare', 'pitch', 'prospect', 'close', 'meeting', 'discovery'],
    workflow: objectionPrepCard,
  },
  'meeting-noshow-reducer': {
    slug: 'meeting-noshow-reducer',
    title: 'Meeting No-Show Reducer',
    outcome: 'Automatically email a friendly confirming reminder to anyone you’re meeting tomorrow — cutting no-shows without lifting a finger.',
    persona: 'sales',
    deliverable_type: 'n8n',
    filename: 'workflowstacks-meeting-noshow-reducer.n8n.json',
    setup_minutes: 6,
    requires: ['A free n8n account (cloud or self-hosted)', 'A Google account (for Sheets)', 'A Gmail account (or swap in Outlook/SMTP)', 'An OpenAI API key not required for this one'],
    match_keywords: ['meeting', 'reminder', 'no-show', 'noshow', 'calendar', 'appointment', 'reduce', 'cancellation', 'booking', 'confirm', 'sales', 'call'],
    workflow: meetingNoshowReducer,
  },
  'ai-support-agent': {
    slug: 'ai-support-agent',
    title: 'AI Support Agent (Chat)',
    outcome: 'A chat agent that answers customer questions from YOUR business info — with conversation memory. It never invents policies; anything it doesn\'t know, it hands to a human.',
    persona: 'founder',
    deliverable_type: 'n8n',
    filename: 'workflowstacks-ai-support-agent.n8n.json',
    setup_minutes: 5,
    requires: ['A free n8n account (cloud or self-hosted)', 'An OpenAI API key (any OpenAI-compatible key works)'],
    match_keywords: ['support', 'chat', 'chatbot', 'agent', 'assistant', 'faq', 'answer', 'questions', 'customer', 'helpdesk', 'website', 'bot'],
    workflow: aiSupportAgent,
  },
}
...
export const PERSONALIZABLE_NODE = {
  ...
  'icp-fit-checker': 'Score fit (AI)',
  'objection-prep-card': 'Build prep (AI)',
  // meeting-noshow-reducer has no AI node — nothing to personalize.
  // ai-support-agent: prompt lives in the agent node's options.systemMessage,
  // not a flat prompt param — excluded until personalize supports that shape.
}
