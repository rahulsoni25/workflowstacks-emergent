...
// Sitemaps Google holds for the property: when each was last downloaded and
// how many URLs it found. "No sitemaps" or a stale lastDownloaded explains
// pages stuck on "URL is unknown to Google" better than any on-page check.
export async function listSitemaps() {
  const json = await api(`${V3}/sites/${encodeURIComponent(siteUrl())}/sitemaps`)
  return (json.sitemap || []).map((m) => ({
    path: m.path, last_submitted: m.lastSubmitted || null, last_downloaded: m.lastDownloaded || null,
    pending: !!m.isPending, errors: Number(m.errors || 0), warnings: Number(m.warnings || 0),
    contents: (m.contents || []).map((c) => ({ type: c.type, submitted: Number(c.submitted || 0), indexed: Number(c.indexed || 0) })),
  }))
}
