...
// Always bypasses Next's data cache (lib/raw-fetch.js): the admin job never
// wanted it, and on the page it only ever produced writes — a skill page
// regenerates every 7 days, by which time the cached tree has expired too.
// fetchOpts is kept for callers that pass headers/signals.
export async function fetchRepoFacts(owner, repo, fetchOpts = {}, { maxSizeKB } = {}) {
  ...
  const metaRes = await rawFetch(base, opts(8000))
  if (!metaRes.ok) {
  ...
  const [langRes, treeRes] = await Promise.all([
    rawFetch(`${base}/languages`, opts(8000)),
    rawFetch(`${base}/git/trees/${encodeURIComponent(branch)}?recursive=1`, opts(15000)),
  ])
