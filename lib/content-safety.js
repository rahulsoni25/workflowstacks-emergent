...
// Narrow, low-false-positive subset of the same signals — specifically about
// hijacking an LLM or a browser (prompt-injection phrasing, XSS payloads,
// hidden/invisible characters used to smuggle instructions). Used for the
// live, per-request scan in lib/claude-skill.js that runs on every compiled
// install (prompt / SKILL.md / zip), including GitHub-scraped content that
// never went through screenSubmission. The broader heuristicScan() signals
// (obfuscated blobs, shortened links, query-injection syntax) are dropped
// here on purpose: ordinary README prose legitimately contains long hashes,
// base64 image data, and badge short-links, and applying those signals to
// thousands of already-published READMEs would start blocking real installs.
