...
export async function listCreators({ compact = false } = {}) {
  ...
  const creators = rows
    ...
    // compact = the public JSON: drop what the client can derive (avatar comes
    // from github.com/<handle>.png) and every false flag. ~40% fewer bytes out.
    .map((c) => {
      if (!compact) return c
      const o = { handle: c.handle, skills: c.skills, stars: c.stars, top_name: c.top_name, top_slug: c.top_slug }
      if (c.org) o.org = true
      if (c.verified) o.verified = true
      if (c.founding) o.founding = true
      return o
    })
  return {
    creators,
...
export async function verifyClaim(rawHandle, ref) {
  ...
  // All (max 5) READMEs are fetched at once with a short timeout, so a claim
  // is one brief function run instead of up to five slow ones in a row.
  const repos = creator.skills.slice(0, 5)
    .map((s) => (s.github_url || '').match(/github\.com\/([^/]+)\/([^/#?]+)/i))
    .filter(Boolean)
    .map((m) => `${m[1]}/${m[2]}`)
  const results = await Promise.all(repos.map(async (full) => {
    try {
      const r = await fetch(`https://api.github.com/repos/${full}/readme`, {
        headers: { ...ghHeaders(), Accept: 'application/vnd.github.raw+json' },
        signal: AbortSignal.timeout(6000),
        cache: 'no-store',
      })
      if (r.status === 403 || r.status === 429) return 'limited'
      if (!r.ok) return null
      return /workflowstacks\.com/i.test(await r.text()) ? full : null
    } catch {
      return null // network hiccup on one repo
    }
  }))
  const matchedRepo = results.find((x) => x && x !== 'limited') || null
  const rateLimited = !matchedRepo && results.includes('limited')
