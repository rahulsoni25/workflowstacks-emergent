...
export async function loadSkill(key) {
  ...
  if (process.env.MONGO_URL) {
    try {
      ...
      // A miss is a miss: the public API reads the same collection, so the
      // HTTP fallback below could only repeat the lookup as a second billable
      // function invocation. Fall through only when the read itself failed.
      return skill || null
    } catch {}
  }
  try {
...
// Same query GET /api/skills?search= runs, executed in-process
// (lib/skills-data.js) so an MCP search_skills call is one function
// invocation, not two.
export async function searchSkills({ query = '', category = '', limit = 8 } = {}) {
  try {
    const data = await listSkills(
      { search: query, category, limit: Math.min(Math.max(1, limit), 20) },
      { revalidate: 600 }
    )
    return data.skills || []
  } catch {
...
export function compileSkillMd(skill) {
  ...
  const quickStart = (guide.quickStart || []).map(stripLeadingOrdinal).filter(Boolean)
  if (quickStart.length) {
    // quickStart is LLM-summarised from the repo description, not extracted
    // from the README, and a live audit caught it inventing file names and
    // CLI flags (a "python claude-seo.py" that does not exist upstream). The
    // installed skill must tell the agent to reconcile every command and
    // path against the upstream README before executing — the README wins.
    if (skill.github_url) {
      lines.push(
        `Ground truth for setup and commands is the project README at ${skill.github_url}. ` +
          'The steps below were machine-summarised by WorkflowStacks and can lag behind or misname files and flags: ' +
          'before running any command or opening any file path from them, confirm it exists in the README or repository tree. ' +
          'Where the two disagree, follow the README and tell the user which step you corrected.'
      )
      lines.push('')
    }
    quickStart.forEach((s, i) => lines.push(`${i + 1}. ${s}`))
  } else {
  ...
  // Skip the Setup block when it would just repeat an Instructions step
  // verbatim — the compiled file shipped the same command twice.
  const installDup = guide.install && quickStart.some((s) => tidySentence(s) === tidySentence(guide.install))
  if ((guide.install && !installDup) || ex.time_to_setup) {
    lines.push('## Setup (if the underlying tool is not installed yet)')
    if (guide.install && !installDup) {
      lines.push('```bash')
