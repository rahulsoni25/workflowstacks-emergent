...
export function starterPrompt(skill, { maxChars = 5500 } = {}) {
  ...
  return [
    ...
    clipped,
    '',
    `To start: ask me what I want to do with ${title}.`,
  ].join('\n')
}
...
export function setupPrompt(skill) {
  ...
  if (skill.github_url) {
    ...
    lines.push('4. Do any configuration the README requires (env vars, API keys) — ask me for values only I can provide.')
    lines.push(`5. Verify it runs end to end, then show me how to use it${goal ? ` for: ${goal}` : '.'}`)
  } else {
    lines.push('Follow the setup instructions in the guide below, verify it works, then show me how to use it.')
  }
  lines.push('')
