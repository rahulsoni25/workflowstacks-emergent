...
import {
  listSkills, getSkillByKey, getStats, getNewsletterIssues,
  getWeeklyLists, hotShareText, skillLabel, applyFallback, escapeRegex,
} from '../../../lib/skills-data';

// CDN cache for public catalog reads (mirrors vercel.json). Set on the
// response itself so it holds regardless of how platform headers merge with
// function output: repeat reads — the catalog grid's filters, crawlers, the
// builder, the MCP connector — are served from the edge instead of
// re-invoking this function.
const PUBLIC_LIST_CACHE = { 'Cache-Control': 'public, s-maxage=600, stale-while-revalidate=3600' };
const PUBLIC_ITEM_CACHE = { 'Cache-Control': 'public, s-maxage=3600, stale-while-revalidate=86400' };
...
// escapeRegex lives in lib/skills-data.js (shared with the page-side readers).
...
// prettyName / prettyDesc / scrubBoilerplate / applyFallback / LIST_PROJECTION
// live in lib/skills-data.js so the pages that read Mongo directly apply the
// exact same fallbacks as this API.
...
// getWeeklyLists / hotShareText (one definition of "hot" for every surface)
// live in lib/skills-data.js; the /hot page and homepage read them directly.
...
export async function GET(request) {
  ...
  try {
    ...
    if (path === '/newsletter/issues') {
      const issues = await getNewsletterIssues({ db: database });
      return Response.json({ issues }, { headers: PUBLIC_LIST_CACHE });
    }
    ...
    if (path === '/skills') {
      // Query building, sorting (incl. 'gems' and the search re-rank) and the
      // field fallbacks are lib/skills-data.js — the same code the catalog
      // pages, sitemap, llms.txt and the MCP connector run in-process, so the
      // API and the site can never disagree about what a listing looks like.
      const { searchParams } = new URL(request.url);
      const out = await listSkills(searchParams, { db: database });
      // all=true (admin view incl. unpublished rows) is never CDN-cached.
      return Response.json(out, searchParams.get('all') === 'true' ? undefined : { headers: PUBLIC_LIST_CACHE });
    }

    // Get skill by slug or ID
    if (path.startsWith('/skills/')) {
      const key = path.split('/')[2];
      const skill = await getSkillByKey(key, { db: database });
      if (!skill) {
        return Response.json({ error: 'Skill not found' }, { status: 404 });
      }
      return Response.json({ skill }, { headers: PUBLIC_ITEM_CACHE });
    }
    ...
    // Stats endpoint
    if (path === '/stats') {
      return Response.json(await getStats({ db: database }), { headers: PUBLIC_LIST_CACHE });
    }
