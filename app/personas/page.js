...
async function getPersonas() {
  try {
    const res = await fetch(`${BASE}/api/personas`, { next: { revalidate: 1800 }, signal: AbortSignal.timeout(10_000) })
    if (!res.ok) return []
  ...
  return (
    <div className="min-h-screen bg-neptune">
      ...
      <div className="container mx-auto px-4 py-12 max-w-6xl">
        ...
        {personas.length === 0 ? (
          <div className="text-center py-20"><p className="text-text-muted text-xl">No personas available yet.</p></div>
        ) : (
              ...
              <Card key={persona.id} className="bg-slate-900/60 border-slate-700/50 backdrop-blur-xl hover:border-teal-500/40 transition-all duration-300">
                <CardHeader>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <Badge variant="outline" className="border-slate-600 text-text-secondary">{persona.skillIds?.length || 0} skills</Badge>
                  </div>
                  <CardTitle className="text-white text-2xl">{persona.name}</CardTitle>
                  <CardDescription className="text-text-muted text-base">{persona.description}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-2 text-sm text-text-muted"><Target className="w-4 h-4" /><span>Best for: {persona.audience}{/(?:s|sh|ch|x|z|y)$/i.test(persona.audience || '') ? '' : 's'}</span></div>
