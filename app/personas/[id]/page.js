...
async function getPersona(id) {
  try {
    const res = await fetch(`${BASE}/api/personas/${id}`, { next: { revalidate: 86400 }, signal: AbortSignal.timeout(10_000) })
    ...
    <div className="min-h-screen bg-neptune">
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl">
        <div className="mb-8">
          ...
          <p className="text-xl text-text-secondary mb-5">{persona.whatItDoes || persona.description}</p>
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="text-sm bg-slate-800/60 border border-slate-700/50 rounded-full px-3 py-1 text-text-secondary">🧩 {persona.skillIds?.length || 0} skills</span>
          ...
          <p className="text-sm text-text-muted mt-2">Loads its {persona.skillIds?.length || 0} skills into the builder and generates a paste-ready agent.</p>
        </div>
        ...
        {persona.brief && (
          <Card className="bg-slate-900/60 border-slate-700/50 mb-8">
            ...
            <CardContent>
              <p className="text-text-secondary italic">"{persona.brief}"</p>
            </CardContent>
          </Card>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {skills.map((skill) => (
                <Card key={skill.id} className="bg-slate-900/60 border-slate-700/50 hover:border-teal-500/40 transition-all">
                  <CardHeader>
                    <Badge className="bg-slate-800 text-text-secondary border-slate-700 w-fit mb-2 text-xs">{skill.category}</Badge>
                    <CardTitle className="text-white text-lg">{skill.title_human || skill.name}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-text-muted text-sm line-clamp-2 mb-3">{skill.description_human || skill.description}</p>
