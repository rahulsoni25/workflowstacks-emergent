        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {m.country && <div className="text-text-muted text-xs flex items-center gap-1"><MapPin className="w-3 h-3" />{m.country}</div>}
            </div>
          </div>
          {m.featured && <Star className="w-4 h-4 fill-amber-400 text-amber-400 shrink-0" />}
        </div>
        <Badge className={`${CAT_COLOR[m.category] || 'bg-slate-700/50 text-text-secondary border-slate-600'} border text-xs mb-2`}>{m.category}</Badge>
        {m.headline && <p className="text-text-secondary text-sm line-clamp-2 mb-1">{m.headline}</p>}
        {m.builds && <p className="text-text-muted text-xs line-clamp-1 mb-3">Builds: {m.builds}</p>}
        ...
        {members.length === 0 ? (
          <Card className="bg-slate-900/60 border-slate-700/50 max-w-lg mx-auto">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-text-secondary text-lg mb-2">Be the first member of the network.</p>
              ...
              return (
                <section key={cat}>
                  <div className="flex items-center justify-between mb-5">
                    <h2 className="text-2xl font-bold text-white">{cat}s <span className="text-text-muted text-lg font-normal">· {inCat.length}</span></h2>
                  </div>
