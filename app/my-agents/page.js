        ...
        {loading ? (
          ...
          <Card className="bg-slate-900/60 border-slate-700/50">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-text-secondary text-lg mb-2">You haven't built any agents yet.</p>
              <p className="text-text-muted mb-6">Pick a few skills and the builder generates a ready-to-paste agent blueprint.</p>
              <Link href="/builder">
                <Button className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white">
              ...
              <Card key={agent.id} className="bg-slate-900/60 border-slate-700/50">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {agent.skills?.map((s) => (
                      <Badge key={s.id} className="bg-slate-800 text-text-secondary border-slate-700">{s.name}</Badge>
                    ))}
                  </div>
