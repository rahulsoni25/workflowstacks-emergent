...
function getCategoryColor(cat) {
  ...
  return colors[cat] || 'bg-slate-500/10 text-text-secondary border-slate-500/20'
}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 to-slate-900">
      <header className="border-b border-slate-700/50 bg-slate-950/80 backdrop-blur-xl">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Button onClick={copyLink} variant="outline" size="sm" className="border-slate-700 text-text-secondary hover:text-white">
      ...
      <div className="container mx-auto px-4 py-10 max-w-5xl">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 text-xs text-text-muted mb-3">
            ...
            <span>An AI Stack by <span className="text-text-secondary font-medium">@{stack.creatorHandle || 'anonymous'}</span></span>
            <span>·</span>
          ...
          <p className="text-xl text-text-secondary mb-4 italic">"{stack.goal}"</p>
          {stack.summary && (
            <div className="bg-teal-500/5 border border-teal-500/30 rounded-md p-4 mb-4">
              ...
              <p className="text-text-secondary">{stack.summary}</p>
            </div>
          )}
          {stack.context && (
            <div className="bg-slate-900/60 border border-slate-700/50 rounded-md p-4 mb-4">
              ...
              <p className="text-text-secondary">{stack.context}</p>
            </div>
          )}
          <div className="flex flex-wrap items-center gap-3 mt-6">
            ...
            <Link href={builderHref}>
              <Button size="lg" variant="outline" className="border-slate-700 text-text-secondary hover:bg-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {skills.map((s, i) => (
            <Card key={s.id} className="bg-slate-900/60 border-slate-700/50 backdrop-blur-xl">
              ...
              <CardContent>
                <p className="text-text-secondary text-sm mb-2">{s.explainer?.what_it_is || s.description_human || s.description}</p>
                {s.explainer?.what_you_can_make && (
                  <p className="text-xs text-text-muted italic">{s.explainer.what_you_can_make}</p>
                )}
        ...
        <div className="mt-12 p-6 bg-gradient-to-r from-violet-500/10 via-cyan-500/10 to-teal-500/10 border border-violet-500/30 rounded-lg text-center">
          ...
          <p className="text-text-secondary mb-4">Tell us what you want to build and we'll recommend the exact skills.</p>
          <Link href="/">
