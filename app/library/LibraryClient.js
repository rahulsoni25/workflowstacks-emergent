  ...
  if (!items.length) {
    return (
      <div className="border border-slate-700/50 rounded-xl bg-slate-900/50 p-10 text-center">
        ...
        <p className="text-text-secondary font-medium mb-1">Nothing saved yet</p>
        <p className="text-text-muted text-sm mb-5">Hit "Save to My Library" on any skill, or ask Claude to install one via the connector.</p>
        <Link href="/skills">
    ...
    <div className="space-y-3">
      {items.map((item) => (
        <div key={item.skill_id} className="flex items-center justify-between gap-3 border border-slate-700/50 rounded-lg bg-slate-900/50 px-4 py-3">
          ...
          <button onClick={() => remove(item.skill_id)} aria-label={`Remove ${item.name}`} className="text-text-muted hover:text-rose-400 transition-colors flex-shrink-0">
